...
impl Scanner {
    ...
    fn scan(&mut self, num_threads: usize) -> String {
        println!("{}", format!("Scanning: {}...", self.file));

        if !self.file.ends_with(".jar") {
            println!("File is not a jar executable!");
            return String::new();
        }
        ...
        match zip.by_name("META-INF/MANIFEST.MF") {
            Ok(mut file) => {
                ...
                if let Some(start) = manifest.find("Main-Class") {
                    ...
                    println!("{}", main_class_info);
                }
            }
            Err(e) => println!("{}", format!("Error processing manifest: {}", e)),
        }
    }
        ...
        for i in 0..zip.len() {
            ...
            if lower_filename.ends_with(".class") {
                ...
                if let Err(e) = file.read_to_end(&mut data) {
                    println!(
                        "{}",
                        format!("Error reading class file {}: {}", filename, e)
                    );
                } else {
                ...
                for (filename, data) in chunk {
                    ...
                    for url_match in url_regex.find_iter(&data_str) {
                        ...
                        if !good_links.iter().any(|g| link.contains(g)) {
                            ...
                            println!("Found link: {} | {}", link, filename);
                        }
                    }
                    ...
                    for ip_match in ip_regex.find_iter(&data_str) {
                        ...
                        println!("Found IP address: {} | {}", ip_address, filename);
                    }
                }
            });
            handles.push(handle);
...
fn main() {
    ...
    print!("Enter the number of threads to use (4): ");
    io::stdout().flush().expect("Failed to flush stdout");
    ...
    if log_to_file {
        ...
        println!("Report saved to scan_report.log");
    }
}
